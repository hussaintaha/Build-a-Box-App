<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
  ...
  function stripeKey($shop){
      $ci      = &get_instance();
      $query   = $ci->db->query("SELECT * FROM `stripe_key` where shop_id='" . shop_id($shop) . "' AND provider='stripe' limit  0,1");
      $rowdata = $query->row();
      if ($rowdata) {
        if ($rowdata->mode == 0){
          return $rowdata->test_s_key;
        }else {
          return $rowdata->live_s_key;
        }
  ...
  function asset_url2($asset_name, $asset_type = null)
{
    $obj            = &get_instance();
  ...
  function CurrentActiveCheckout($shop_id)
  {
    $ci =& get_instance();
    $shop_id = $ci->db->select('*')->where('shop_id',$shop_id)->get('tbl_active_checkout')->row();
    if($shop_id){
      return $shop_id->current_checkout;
    }else{
      return 0;
    }
  }
  function GetRechargeKey($shop_id)
  {
    $ci =& get_instance();
    $shop_id = $ci->db->select('*')->where('shop_id',$shop_id)->get('recharge_api_keys')->row();
    if($shop_id){
      return $shop_id->recharge_token;
    }
  }
  ...
  function mode($shop)
  {
      ...
      $ci =& get_instance();
      $mode = $ci->db->select('*')->where('shop_id', $shop_id)->get('stripe_key')->row();
      return $mode->mode;
  }


  function GetStripeKeys($shop_id)
  {
    $ci =& get_instance();
    $shop_id = $ci->db->select('*')->where('shop_id',$shop_id)->get('stripe_key')->row();
    if($shop_id){
      return $shop_id;
    }else{
      return false;
  ...
  {
    $ci =& get_instance();
    $shop_id = $ci->db->select('*')->where('shop',$shop)->get('shopify_stores')->row();
    return $shop_id->id;
  }
  function EndpointSecret($mode,$shop_id)
  {
    $ci =& get_instance();
    $shop_id = $ci->db->select('secret')->where(['mode'=>$mode,'shop_id'=>$shop_id])->get('stripe_webhook_endpoints')->row();
    return $shop_id->secret;
  }
  function CountSForDashboard($table,$shop_id)
  {
    $ci =& get_instance();
    $num = $ci->db->select('*')->where(['shop_id'=>$shop_id])->get($table);
    return $num->num_rows();
  }
