              ...
              <tbody>
                <?php foreach ($Boxes as $key => $box) { ?>
                  ...
                  <td style="text-align:center" class="Polaris-DataTable__Cell Polaris-DataTable__Cell--verticalAlignTop Polaris-DataTable__Cell--firstColumn" scope="row">
                    <a href="<?=$box->bx_before_image?>" target="_blank">
                      <img style="height:50px;width:50px" src="<?=$box->bx_before_image?>">
                    </a>
                  </td>
                  ...
                  <td class="Polaris-DataTable__Cell Polaris-DataTable__Cell--verticalAlignTop Polaris-DataTable__Cell--numeric">
                    <?php
                    foreach (explode(',',$box->bx_colors) as $color) { ?>
                      <div class="color swatch-box" style="background:<?=$color?>"></div>
                    <?php }
                      ?>
                  </td>
                  <td class="Polaris-DataTable__Cell Polaris-DataTable__Cell--verticalAlignTop Polaris-DataTable__Cell--numeric">
                  ...
                  <td style="text-align:center"class="Polaris-DataTable__Cell Polaris-DataTable__Cell--verticalAlignTop Polaris-DataTable__Cell--numeric">
                    <?php if($box->bx_contents_based_on_product > 0){ ?>
                      <p><b>No of Products </b><?=$box->bx_max_no_products?></p>
                    <?php }?>
                    <?php if($box->bx_contents_based_on_bx_amt > 0){ ?>
                      <p><b>Box Products Total </b><?=$box->bx_max_amt?></p>
                    <?php }?>
                  </td>
                  <td class="Polaris-DataTable__Cell Polaris-DataTable__Cell--verticalAlignTop Polaris-DataTable__Cell--numeric">
                        <label class="switch">
                  ...
                  <td class="Polaris-DataTable__Cell Polaris-DataTable__Cell--verticalAlignTop Polaris-DataTable__Cell--numeric">
                        <a style="text-decoration:none;" href="<?=base_url()."Home/LoadEditPage?bx_id=".$box->id."&shop=".$shop."&shop_id=".$shop_id?>">
...
$('.chkbs').on('change',function(){
    ...
    $.ajax({
        type: 'POST',
        url: UpDateURL,
        data: {id : $(this).data('id'),status:NewStatus,table:'tbl_box'},
        dataType: 'json',
...
$('.cofirmDelete').on('click',function(){
  ...
  $.ajax({
      type: 'POST',
      url: DeleteURL,
      data: {id:window.deleteId,table:'tbl_box'},
      dataType: 'json',
      success: function(response){
          $('.BXLOADER').hide();
          ShopifyApp.flashNotice(response.msg);
